...
HS_INIT_SEQ = 0xb8
# Before switching from HS to LP mode, lane must be kept in HS-Trail for at least
# max(8*UI, 60 ns + 4*UI) nanosecods, where UI is D-PHY clock period (3.367ns)
HS_TRAIL_CYCLES = 12
...
class PacketFormatter(Module):
    ...
    def __init__(self, four_lanes=False):
        ...
        # proceed with long packet or generate End-of-Transmission if it's a short
        # packet
        ...
        fsm.act("EoT",
            ...
            If(payload_cnt == 0,
                ...
                # HS Trail, invert MSB of each byte and keep lane in such state
                self.data_o.eq(Cat(Replicate(~last_data[7], 8), Replicate(~last_data[-1], 8))),
            ).Elif(payload_cnt != HS_TRAIL_CYCLES,
                self.data_o.eq(self.data_o),
            ).Else(
